...
const copy = (el) => {
  ...
  if (options.t && options.t.length) {
    ...
    form.childNodes.forEach((node) => {
      ...
      string = string.trim();
      if (string.length) strings.push(string.trim());
    });
...
function renderContent(time, seed) {
  ...
  const gridLayout = (emojis, options = {}) => {
    ...
    let stagger =
      pattern == "diamond" || pattern == "hex" || pattern == "random";
    ...
    for (var y = 0; y < rows; y++) {
      for (var x = 0; x < cols; x++) {
        ...
        let randomScale = pattern == "random" ? 0.5 : 0.0;
        ...
        ctx.save();
        ctx.translate(
          marginX + (spacingX + size) * (x + rx + staggerX),
  ...
  const foamLayout = (emojis, options = {}) => {
    ...
    for (var j = 0; j < 10000; j++) {
      ...
      for (var i = 0; i < bubbs.length; i++) {
        r = Math.min(
          r,
          Math.hypot(x - bubbs[i].x, y - bubbs[i].y) -
            bubbs[i].r -
            (size * spacing) / 4
        );
        if (r < 5) break;
  ...
  const spiralLayout = (emojis, options = {}, varyScale = false) => {
    ...
    for (var i = 0; i < 100000; i++) {
      ...
      let randomScale = pattern == "random" ? 1.0 : 0.0;
      let rx = (random() - 0.5) * randomScale;
      let ry = (random() - 0.5) * randomScale;

      var x =
        marginX + width / 2 + scale * r * Math.cos(a) * spacing + size * rx;
      var y =
        marginY + height / 2 + scale * r * Math.sin(a) * spacing + size * ry;
  ...
  emojiCanvas = emojis.map((emoji) => {
    ...
    imgctx.fillText(
      emoji,
      imgcanvas.width / 2,
      imgcanvas.height / 2 + 0.375 * iconSize
    );
    return imgcanvas;
  });
